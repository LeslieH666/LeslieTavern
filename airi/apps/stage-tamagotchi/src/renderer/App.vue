...
import { applyLeslieCompanionDisplayDefault, useLeslieCompanionDefaults } from './composables/leslie-companion-defaults'
...
useAuthProviderSync()
const applyLeslieCompanionDefaults = useLeslieCompanionDefaults()
...
function createFullStageRuntime() {
  ...
  return {
    async initialize() {
      ...
      applyLeslieCompanionDisplayDefault(settingsStore)
      cardStore.initialize()
      applyLeslieCompanionDefaults()
...
onMounted(async () => {
  ...
  // The transparent companion window is useful as soon as its display model
  // is ready. Chat history hydration is non-authoritative in Leslie companion
  // mode and must not prevent the Live2D stage from appearing if IndexedDB is
  // slow or wedged after an interrupted Electron run.
  try {
    await fullStageRuntime?.initialize()
  }
  catch (error) {
    console.error('[App] Failed to initialize the full stage runtime:', error)
  }

  void chatSessionStore.initialize().catch((error) => {
    console.warn('[App] Chat session hydration failed; the stage remains available:', error)
  })
})

...
import { setupLeslieBridge } from './services/airi/leslie-bridge'
...
app.whenReady().then(async () => {
  ...
  const leslieBridge = injeca.provide('services:leslie-bridge', () => {
    const { context } = createContext(ipcMain)
    const service = setupLeslieBridge({ context })
  ...
  const mainWindow = injeca.provide('windows:main', {
    dependsOn: { editorWindow, settingsWindow, chatWindow, widgetsManager, noticeWindow, beatSync, autoUpdater, serverChannel, godotStageManager, mcpStdioManager, i18n, onboardingWindowManager, windowAuthManager, leslieBridge },
  ...
  injeca.invoke({
    dependsOn: { mainWindow, tray, serverChannel, airiHttpServer, godotStageManager, pluginHost, mcpStdioManager, onboardingWindow: onboardingWindowManager, widgetsWindow: widgetsManager, spotlightWindow, artistryConfig, leslieBridge },
    callback: async (deps) => {
      const { context } = createContext(ipcMain)

...
export async function setupMainWindow(params: {
  ...
  // The Leslie launcher runs the source app through an unpackaged Electron
  // runtime, which Electron reports as development mode. Only open DevTools
  // when the operator explicitly requests it; a detached inspector must not
  // become part of normal companion startup.
  ...
  // Transparent panel windows do not always emit `ready-to-show` on Windows.
  // Loading has completed here, so ensure AIRI cannot remain hidden forever.
  if (!window.isDestroyed() && !window.isVisible()) {
    console.warn('AIRI main window loaded without ready-to-show; showing it explicitly.')
    window.show()
  }

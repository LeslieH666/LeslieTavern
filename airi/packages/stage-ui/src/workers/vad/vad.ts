...
export class VAD implements BaseVAD {
  ...
  public async initialize(): Promise<void> {
    try {
      ...
      // Silero exposes a custom ONNX graph, so load the generic model directly.
      // AutoModel reaches the same fallback after printing a misleading warning.
      this.model = await PreTrainedModel.from_pretrained('onnx-community/silero-vad', { config: { model_type: 'custom' } as any, dtype: 'fp32' })
      this.isReady = true

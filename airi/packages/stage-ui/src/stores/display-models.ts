...
const DISPLAY_MODELS_LOAD_TIMEOUT_MS = 5_000
...
export const useDisplayModelsStore = defineStore('display-models', () => {
  ...
  async function loadDisplayModelsFromIndexedDB() {
    ...
    try {
      // NOTICE:
      // Chromium can leave the initial localforage request pending forever
      // after an interrupted Electron renderer. The built-in Live2D presets
      // are sufficient to render a usable stage, so storage hydration is
      // best-effort and must never hold the character window hostage.
      // Source/context: AIRI main renderer startup on Windows after an
      // unclean restart; localforage.iterate never settles.
      // Removal condition: persisted model hydration has a cancellable or
      // otherwise bounded IndexedDB implementation.
      await Promise.race([
        localforage.iterate<{ format: DisplayModelFormat, file: File, importedAt: number, previewImage?: string }, void>((val, key) => {
        ...
        new Promise<void>((_resolve, reject) => {
          timeoutId = setTimeout(() => reject(new Error(`Display model storage did not respond within ${DISPLAY_MODELS_LOAD_TIMEOUT_MS}ms`)), DISPLAY_MODELS_LOAD_TIMEOUT_MS)
        }),
      ])
    }
    catch (err) {
      console.warn('[display-models] persisted models unavailable; continuing with built-in presets:', err)
    }
    finally {

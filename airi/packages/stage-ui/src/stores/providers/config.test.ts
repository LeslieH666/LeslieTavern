...
describe('provider config store', () => {
  ...
  it('returns a cloneable value when it ensures an existing provider', () => {
    const store = installStore()

    store.ensureProvider(localProvider.id, localProvider.definitionId, localProvider.config)
    const result = store.ensureProvider(localProvider.id, localProvider.definitionId, localProvider.config)

    // ROOT CAUSE:
    //
    // The old action returned a Vue proxy from persisted provider state.
    // The synced Pinia action tried to clone that proxy across Electron windows.
    // This caused the reported DataCloneError during Leslie companion startup.

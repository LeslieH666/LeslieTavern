...
export const useChatSessionStore = defineStore('chat-session', () => {
  ...
  async function loadIndexForUser(currentUserId: string) {
    ...
    // Hydrate metadata without pulling every session's full message history.
    // Existing entries win so mutations made while the index was loading are
    // not overwritten by the persisted snapshot.
    if (index.value) {
  ...
  async function persistSession(sessionId: string) {
    await enqueuePersist(async () => {
      ...
      if (index.value)
        await chatSessionsRepo.saveIndex(cloneDeep(index.value))
    })
  }
  ...
  async function loadSession(sessionId: string) {
    ...
    const loadPromise = (async () => {
      try {
        ...
        // A delete or account switch may have removed the session while the
        // IndexedDB read was pending. Do not resurrect that stale record.
        if (!sessionMetas.value[sessionId])
          return
        ...
        if (sessionMetas.value[sessionId]?.cloudChatId)
  ...
  async function deleteSession(sessionId: string) {
    ...
    // ROOT CAUSE:
    // Mutate memory and IndexedDB before issuing the remote delete. Otherwise
    // an overlapping persist can snapshot the doomed entry and resurrect it.

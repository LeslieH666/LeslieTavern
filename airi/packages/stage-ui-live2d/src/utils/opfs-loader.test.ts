...
describe('opfs cache full directory persistence', () => {
  ...
  it('reads an asar file response through arrayBuffer without waiting on OPFS', async () => {
    const zipBlob = await createZip({
      'model.model3.json': JSON.stringify({
        Version: 3,
        FileReferences: { Moc: 'model.moc3', Textures: ['texture.png'] },
    ...
    expect(context.opfsKey).toBeUndefined()
  ...
  it('continues with the source archive when an OPFS lookup never settles', async () => {
    vi.useFakeTimers()
    vi.stubGlobal('navigator', {
      storage: {
        getDirectory: vi.fn(() => new Promise(() => {})),
      },
    })
    const sourceBlob = new Blob([new Uint8Array([1, 2, 3])], { type: 'application/zip' })
    vi.stubGlobal('fetch', vi.fn(async () => new Response(sourceBlob)))
    const context = {
      source: { id: 'stalled-cache-model', url: 'blob:source' },
    } as Parameters<typeof OPFSCache.checkMiddleware>[0]
    const next = vi.fn(async () => {})

    const checking = OPFSCache.checkMiddleware(context, next)
    await vi.advanceTimersByTimeAsync(1_500)
    await checking

    expect(next).toHaveBeenCalledTimes(1)
    expect(context.opfsZipBlob).toBeDefined()
    vi.useRealTimers()
  })

  it('does not wait for cache persistence before completing the model pipeline', async () => {
    const pendingSave = new Promise<void>(() => {})
    const save = vi.spyOn(OPFSCache, 'save').mockReturnValue(pendingSave)
    const next = vi.fn(async () => {})
    const context = {
      opfsKey: 'non-blocking-model',
      opfsUrl: 'blob:source',
      opfsZipBlob: new Blob([new Uint8Array([1])]),
    } as Parameters<typeof OPFSCache.saveMiddleware>[0]

    await OPFSCache.saveMiddleware(context, next)

...
const live2DOpfsLookupTimeoutMs = 1_500
...
export class OPFSCache {
  ...
  static checkMiddleware: Middleware<OPFSContext> = async (context, next) => {
    ...
    // NOTICE:
    // Bundled file URLs are already durable and can be read directly. Avoid
    // touching OPFS for them: a wedged Chromium storage backend otherwise
    // leaves Live2DFactory pending forever before it even reads the ZIP.
    // Source/context: Leslie companion's bundled Hiyori ZIP on Windows.
    // Removal condition: OPFS directory access is guaranteed to settle and
    // bundled file responses no longer need the arrayBuffer compatibility path.
    const shouldUsePersistentCache = !blobUrl.startsWith('file:')
    let cacheLookupTimeout: ReturnType<typeof setTimeout> | undefined
    let cacheLookupTimedOut = false
    const files = shouldUsePersistentCache
      ? await Promise.race([
          OPFSCache.get(key, blobUrl),
          new Promise<null>((resolve) => {
            cacheLookupTimeout = setTimeout(() => {
              cacheLookupTimedOut = true
              resolve(null)
            }, live2DOpfsLookupTimeoutMs)
          }),
        ]).finally(() => {
          if (cacheLookupTimeout !== undefined)
            clearTimeout(cacheLookupTimeout)
        })
      : null

    if (cacheLookupTimedOut)
      console.warn(`[OPFS] Cache lookup timed out for ${key}; loading the source archive directly`)
    ...
    context.opfsKey = shouldUsePersistentCache ? key : undefined
  ...
  static saveMiddleware: Middleware<OPFSContext> = async (context, next) => {
    const cacheKey = context.opfsKey
    const cacheBlob = context.opfsZipBlob
    const cacheUrl = context.opfsUrl

    // Finish the model pipeline first. OPFS is an optimization and must not
    // hold the visible character or its controls in the loading state.
    await next()

    if (cacheKey && cacheBlob)
      void OPFSCache.save(cacheKey, cacheBlob, cacheUrl)
  }
}

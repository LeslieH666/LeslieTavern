...
async function loadModel() {
  ...
  // The mutex is acquired above and released in the single finally below, which
  // covers every code path: the pixi-wait timeout, an empty model source, an
  // unmount mid-load, a setup error, and the success path. The whole guarded
  // body stays inside ONE try/finally so a future early return cannot leak the
  // lock again — a leaked lock leaves the stage stuck on Loading forever
  // because the next loadModel() blocks on modelLoadMutex.acquire().
  try {
    if (!pixiApp.value || !pixiApp.value.stage) {
      try {
        ...
        // throwOnTimeout is required: VueUse's until() RESOLVES on timeout by
        // default, and without it we would proceed without a stage and crash on
        // pixiApp.value!.stage.addChild() below, leaving a half-loaded model.
        await until(() => !!pixiApp.value && !!pixiApp.value.stage)
    ...
    try {
      await initExpressionController(internalModelRef.value)
    }
    catch (error) {
      console.warn('[Model.vue] Expression controller initialization failed:', error)
    }
    finally {

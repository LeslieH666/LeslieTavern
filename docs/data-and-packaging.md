...
The root Windows shortcuts call scripts under `packaging/windows-local/`. Paths are resolved relative to the repository, so the project can be moved without editing the scripts. `启动本地模型.cmd` and `关闭本地模型.cmd` manage only the tracked project KoboldCpp process and its child worker; an untracked process occupying port 5001 is never replaced. The launcher verifies the actual data root and the listener resolved from `Config/config.yaml` before reporting success. When LAN web access is enabled, `whitelistDirectPrivateNetworks` permits only clients that share the private subnet of the local address they connected to, so Wi-Fi changes do not require per-device entries and unrelated routed subnets are not implicitly trusted. Keep the Windows firewall restricted to a trusted local-subnet boundary; portable builds remain localhost-only.
